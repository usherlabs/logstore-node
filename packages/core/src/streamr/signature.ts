...
export const createSignaturePayload = (opts: {
	...
	if (
		opts.signatureType === SignatureType.SECP256K1 ||
		opts.signatureType === SignatureType.ERC_1271
	) {
		const header = Buffer.concat([
			...
			opts.prevMsgRef !== undefined
				? Buffer.from(
						`${opts.prevMsgRef.timestamp}${opts.prevMsgRef.sequenceNumber}`
					)
				: new Uint8Array(0),
		]);
		if (opts.messageType === StreamMessageType.MESSAGE) {
			const newGroupKeyId = opts.newGroupKey
				? Buffer.from(opts.newGroupKey.id)
				: undefined;
			...
			return Buffer.concat([
				...
				Buffer.concat(
					response.groupKeys.map((k) =>
						Buffer.concat([utf8ToBinary(k.id), k.data])
					)
				),
			]);
		} else {
			throw new Error(
				`Assertion failed: unknown message type ${opts.messageType}`
			);
		}
