...
export interface ISocketClient {
	 ...
	 */
	subscribeToProofs(
		input: TlsProofFilter,
		options?: RpcOptions
	): ServerStreamingCall<TlsProofFilter, TlsProof>;
	/**
	...
	validate(
		input: TlsProof,
		options?: RpcOptions
	): UnaryCall<TlsProof, ValidationResult>;
}
/**
 * @generated from protobuf service socket.Socket
 */
export class SocketClient implements ISocketClient, ServiceInfo {
	...
	subscribeToProofs(
		...
		return stackIntercept<TlsProofFilter, TlsProof>(
			'serverStreaming',
			this._transport,
			method,
			opt,
	...
	validate(
		...
		const method = this.methods[1],
			opt = this._transport.mergeOptions(options);
		return stackIntercept<TlsProof, ValidationResult>(
			'unary',
			this._transport,
			method,
			opt,
