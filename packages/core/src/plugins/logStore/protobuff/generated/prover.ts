...
export interface TlsProof {
    ...
    process: string;
    /**
     * @generated from protobuf field: string notary = 5;
     */
    notary: string; // bool publish = 4;
}
/**
...
class TlsProof$Type extends MessageType<TlsProof> {
    constructor() {
        super("socket.TlsProof", [
            ...
            { no: 5, name: "notary", kind: "scalar", T: 9 /*ScalarType.STRING*/ }
        ]);
    }
    ...
    internalBinaryRead(reader: IBinaryReader, length: number, options: BinaryReadOptions, target?: TlsProof): TlsProof {
    ...
    internalBinaryWrite(message: TlsProof, writer: IBinaryWriter, options: BinaryWriteOptions): IBinaryWriter {
        /* string id = 1; */
        if (message.id !== "")
        ...
        /* string notary = 5; */
        if (message.notary !== "")
            writer.tag(5, WireType.LengthDelimited).string(message.notary);
...
class TlsProofFilter$Type extends MessageType<TlsProofFilter> {
    ...
    internalBinaryRead(reader: IBinaryReader, length: number, options: BinaryReadOptions, target?: TlsProofFilter): TlsProofFilter {
        return target ?? this.create();
    }
    internalBinaryWrite(message: TlsProofFilter, writer: IBinaryWriter, options: BinaryWriteOptions): IBinaryWriter {
        let u = options.writeUnknownFields;
        if (u !== false)
            (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
        return writer;
...
export const Socket = new ServiceType("socket.Socket", [
    { name: "subscribeToProofs", serverStreaming: true, options: { "socket.zmq_type": "sub" }, I: TlsProofFilter, O: TlsProof },
    { name: "validate", options: { "socket.zmq_type": "reply" }, I: TlsProof, O: ValidationResult }
]);
